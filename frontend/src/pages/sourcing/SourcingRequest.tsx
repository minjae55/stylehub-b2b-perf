...
interface SourcingItem {
    ...
    mainCategory: string;
    subCategory: string;
    subCategoryId: number | null; // TODO: 카테고리 API 연동 시 실제 ID로 교체
    unitPrice: string;
    refUrl: string;
...
// 1단계: JSON 전송 → sourcingRequestIds 배열 반환
async function postSourcingRequests(
    buyerId: number,
    items: SourcingItem[]
): Promise<number[]> {
    const body = {
    ...
    return data.sourcingRequestIds as number[];
}
...
function SourcingCard({
    ...
    const updateReadyOptionField = (rid: string, field: "quantity" | "sampleQuantity", val: string) =>
        onChange(
            item.id,
            "readyOptions",
            item.readyOptions.map((r) => r.id === rid ? { ...r, [field]: val } : r),
        );
    ...
    const removeOptionPair = (rid: string, pairIndex: number) =>
        onChange(
            ...
            item.readyOptions.map((r) =>
                r.id === rid
                    ? { ...r, pairs: r.pairs.filter((_, i) => i !== pairIndex) }
                    : r
            ),
        );
        ...
        onChange(
            ...
            item.readyOptions.map((r) =>
                r.id === rid
                    ? {
                        ...r,
                        pairs: r.pairs.map((p, i) => i === pairIndex ? { ...p, [key]: val } : p),
                    }
                    : r
            ),
        );
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {field("대카테고리", true,
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        onChange(item.id, "subCategoryId", null);
                                    }}
                                    className={inputCls}
                                >
                            ...
                            {field("중카테고리", true,
                                <select
                                    ...
                                    onChange={(e) => {
                                        onChange(item.id, "subCategory", e.target.value);
                                        // TODO: 카테고리 API 연동 시 실제 subCategoryId로 교체
                                        onChange(item.id, "subCategoryId", null);
                                    }}
                                    className={inputCls}
                        ...
                        {field("주소재", false,
                            <input
                                ...
                                onChange={(e) => onChange(item.id, "mainMaterial", e.target.value)}
                        ...
                        {field("레퍼런스 URL", false,
                            <input
                                value={item.refUrl}
                                onChange={(e) => onChange(item.id, "refUrl", e.target.value)}
...
export function SourcingRequest() {
    ...
    const canSubmit = items.every(isItemValid) && !isLoading;
    ...
    const handleSubmit = async () => {
        ...
        try {
            // 1단계: JSON 전송
            const sourcingRequestIds = await postSourcingRequests(DUMMY_BUYER.buyerId, items);

            // 2단계: 파일 업로드 (항목별로 매핑)
            await Promise.all(
                items.map(async (item, index) => {
                    const requestId = sourcingRequestIds[index];
                    if (!requestId) return;

                    if (item.type === "READY" && item.refImageFile) {
                        await uploadFiles(requestId, "REF_IMAGE", [item.refImageFile]);
                    }
                    if (item.type === "CUSTOM" && item.workFiles.length > 0) {
                        await uploadFiles(requestId, "WORK_FILE", item.workFiles);
                    }
                })
            );

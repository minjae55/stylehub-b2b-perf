...
import {
  ...
  Search, Filter, FileText, ChevronDown, ChevronUp,
} from "lucide-react";
...
interface CustomRequest {
  ...
  buyerName: string;
  needSample: "Y" | "N";
  workFileCount: number; // 실제로는 파일 URL 목록이겠지만 더미라 카운트만
}
...
interface CustomBidForm {
  unitPrice: string;       // 제작 단가 (벌당)
  samplePrice: string;     // 샘플비
  availableDate: string;
  quoteFile: File | null;  // 견적서 PDF
  comment: string;
}
...
function ReadyBidModal({
  ...
  const handleSubmit = () => {
    onSubmit({ requestId: request.id, type: "READY", unitPrice: form.unitPrice, availableDate: form.availableDate, comment: form.comment });
    setStep("done");
  };
        ...
        {step === "done" ? (
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <div className="bg-secondary rounded-lg px-4 py-3 text-sm text-foreground whitespace-pre-line leading-relaxed">
                {request.detail || "—"}
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                onClick={() => setStep("bid")}
                className="flex-1 py-2.5 bg-primary hover:bg-primary/90 text-white rounded text-sm font-bold transition-colors flex items-center justify-center gap-2"
              >
...
function CustomBidModal({
  ...
  const [form, setForm] = useState<CustomBidForm>(
    existingBid
      ? { unitPrice: existingBid.unitPrice ?? "", samplePrice: existingBid.samplePrice ?? "", availableDate: existingBid.availableDate ?? "", quoteFile: existingBid.quoteFile ?? null, comment: existingBid.comment ?? "" }
      : { unitPrice: "", samplePrice: "", availableDate: "", quoteFile: null, comment: "" }
  );

  const inputCls = "w-full border border-border rounded px-3 py-2.5 text-sm outline-none focus:border-primary transition-colors bg-white";
  const canSubmit = !!(form.unitPrice && form.availableDate && form.quoteFile);

  const handleSubmit = () => {
    onSubmit({ requestId: request.id, type: "CUSTOM", unitPrice: form.unitPrice, samplePrice: form.samplePrice, availableDate: form.availableDate, quoteFile: form.quoteFile, comment: form.comment });
    setStep("done");
  };
        ...
        {step === "done" ? (
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${request.needSample === "Y" ? "bg-primary/5 text-primary border-primary/30" : "bg-secondary text-muted-foreground border-border"}`}>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">총 예산 <span className="text-primary">*</span></label>
                <div className="relative">
                  <input type="number" value={form.unitPrice} onChange={(e) => setForm({ ...form, unitPrice: e.target.value })} placeholder="예: 5,000,000" className={`${inputCls} pr-8`} />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">원</span>
                </div>
              </div>
              {request.needSample === "Y" && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">샘플비 <span className="text-muted-foreground font-normal text-xs">(선택)</span></label>
                  <div className="relative">
                    <input type="number" value={form.samplePrice} onChange={(e) => setForm({ ...form, samplePrice: e.target.value })} placeholder="예: 80,000" className={`${inputCls} pr-8`} />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">원</span>
                  </div>
                </div>
              )}
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">
                견적서 PDF <span className="text-primary">*</span>
              </label>
              {form.quoteFile ? (
                <div className="flex items-center gap-3 border border-primary/40 bg-primary/5 rounded px-3 py-2">
                  ...
                  <button onClick={() => setForm({ ...form, quoteFile: null })} className="text-muted-foreground hover:text-red-500 transition-colors">
                    <X size={14} />
                  </button>
                </div>
              ) : (
...
// ── 주문제작 요청 리스트 행 ────────────────────────────────────────────
function CustomRow({ req, myBid, onBid }: { req: CustomRequest; myBid: MyBid | null; onBid: (r: CustomRequest) => void }) {
  const dday = getDday(req.deliveryDate);
  const ddayIcon = dday.level === "urgent" ? <Zap size={11} /> : dday.level === "soon" ? <AlertTriangle size={11} /> : <Calendar size={11} />;
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {myBid && (
              <span className="text-[10px] bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded-full font-semibold">제안완료</span>
            )}
          </div>
        ...
        <div className="hidden md:flex items-center gap-6 text-center flex-shrink-0">
          ...
          <div>
            <div className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${req.needSample === "Y" ? "bg-primary/5 text-primary border-primary/30" : "bg-secondary text-muted-foreground border-border"}`}>
...
export function SellerRequestList() {
  ...
  const filteredReady = READY_DUMMY.filter((r) => {
  ...
  const filteredCustom = CUSTOM_DUMMY.filter((r) => {
    const matchSearch = r.productName.includes(search) || r.detail.includes(search);
    const matchCat = activeCategory === "전체" || r.category === activeCategory;
        ...
        <div className="bg-white border border-border rounded p-5 mb-6">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="text-xs text-muted-foreground flex items-center gap-1 ml-auto">
              <Filter size={12} /> {activeTab === "READY" ? filteredReady.length : filteredCustom.length}건
            </div>
          </div>
        ...
        {activeTab === "CUSTOM" && (
          <>
            <div className="space-y-3">
              {filteredCustom.map((req) => (
                <CustomRow key={req.id} req={req} myBid={myBids[req.id] ?? null} onBid={setSelectedCustom} />
              ))}
            </div>

    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded p-3 flex items-start gap-2">
                ...
                <p className="text-xs text-amber-700 leading-relaxed">직원으로 가입하시는
                    경우 <strong>대표자</strong>에게 <strong>승인</strong>을 받으셔야 계정이 활성화 됩니다.</p>
            </div>
            <div className="flex items-center gap-1.5 text-sm font-semibold text-foreground"><User size={14}
                                                                                                   className="text-primary"/> 계정
                정보
            </div>
            ...
            <Field label="비밀번호" required>
                <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPw(!showPw)}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">{showPw ?
            ...
            <Field label="비밀번호 확인" required>
                <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowConfirm(!showConfirm)}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">{showConfirm ?
                        <EyeOff size={16}/> : <Eye size={16}/>}</button>
                </div>
...
function Step2({form, set}: { form: FormData; set: (f: Partial<FormData>) => void; }) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-1.5 text-sm font-semibold text-foreground"><Building2 size={14}
                                                                                                        className="text-primary"/> 사업자
                정보
            </div>
...
function Step3({form, set}: { form: FormData; set: (f: Partial<FormData>) => void }) {
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-1.5 text-sm font-semibold text-foreground"><MapPin size={14}
                                                                                                     className="text-primary"/> 기본
                배송지
            </div>
            <p className="text-xs text-muted-foreground bg-muted/40 rounded p-3 -mt-1">사업장 주소와 다를 수 있으므로 실제 상품을 수령할 주소를
                입력해 주세요.</p>
...
export function RegisterBuyer() {
    ...
    const [form, setForm] = useState<FormData>({
        ...
        businessNo: "", companyName: "", presidentName: "", businessLicense: null,
    ...
    return (
        <div>
            ...
            <div className="mb-4 mt-2">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full "president"`}>대표자 가입</span>
            </div>

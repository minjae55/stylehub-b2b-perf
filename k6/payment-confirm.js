...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{name:payment_confirm}': ['p(95)<800'],
  },
};
...
export default function () {
  ...
  const orderRes = http.post(
    ...
    const confirmRes = http.post(
      ...
      JSON.stringify({
        paymentKey: `loadtest-${__VU}-${Date.now()}`,
        orderId: orderData.orderNos[0],
        orderIds: orderData.orderNos,
    ...
    check(confirmRes, { '결제 confirm 성공': (r) => r.status === 200 });
}